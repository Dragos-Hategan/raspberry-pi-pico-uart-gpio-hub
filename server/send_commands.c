...
static inline void server_set_device_state(uart_pin_pair_t pin_pair, uart_inst_t* uart_instance, uint32_t device_index, bool device_state, uint32_t flash_client_index){
    server_send_device_state(pin_pair, uart_instance, device_index, device_state);
    ...
    state_copy.clients[flash_client_index].running_client_state.devices[device_index].is_on = device_state;
...
static void server_read_client_and_device_data(){ 
    ...
    server_set_device_state(active_uart_server_connections[client_index - 1].pin_pair, active_uart_server_connections[client_index - 1].uart_instance, device_index, device_state, flash_client_index);
}
